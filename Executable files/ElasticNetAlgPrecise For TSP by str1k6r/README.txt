...
!!!!!AWARE!!!!! All fractional number must be written with PERIOD, not the comma!!!!
!!!!!Another important note: if program fails once, it wouldn't launch anymore in this session so the only solution is to shut the program and run it again!!!!
...
Ver 1.7.3
	...
	Some minor misprints were corrected, such as repetition of 'Real distance is Real distance is' in final dialog
-----------------------------------------------------------------------------

Ver 1.9.2

	This version differs from the precedent one in such ways:
 	-a plenty of code was optimized and rewritten;
	-algorithm works and calculates separate iterations much faster;
	-algorithm avoids much more deadlocks and impasses;
	-now algorithm is able to create much better ways between cities 
	-algorithm also turns down many cases of algorithm' incertitude in choise between two corresponding nodes;
	-the only minus is the not seldom cases of algorithm slowdown which can be significant enough in some cases.
 	That's because algorithm uses more precise calculation than original and even code optimization 
	and speed-up sometimes can't save the speed fully;
